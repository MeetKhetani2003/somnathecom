...
async function run() {
  ...
  for (const user of users) {
    ...
    if (user.wishlist) {
      for (const w of user.wishlist) {
         // wishlist might just be an array of IDs, let's check
         if (typeof w === 'object' && w.productId) {
           if (!productMap.has(w.productId)) {
             productMap.set(w.productId, { id: w.productId, title: w.title, image: w.image, price: w.price });
           }
         }
      }
    }
  ...
  console.log("Recoverable Products from Users:", recoveredProducts.length);
